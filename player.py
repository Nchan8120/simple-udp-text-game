...
#discovery service port number

disc_port = ("localhost", 4444)
...
def process_command(command):
    ...
    # Send command to server, if it isn't a local only one. If the sever doesn't respond in 5 seconds end the program
    ...
    try:
        ...
        # direction.  We can then join the new room as we know we've been dropped already from the other one.
            ...
            response, addr = client_socket.recvfrom(1024)
            message = "LOOKUP " + str(response.decode())
            client_socket.sendto(message.encode(), disc_port)
            ...
            if (words[0] == "NOTOK"):
                ...
                try:
                    ...
                    if ((server_address.scheme != 'room') or (server_address.port == None) or (
...
def main():
    ...
    parser.add_argument("server", help="name of the room the player is joining")
    args = parser.parse_args()

    # Check the URL passed in and make sure it's valid.  If so, keep track of
    # things for later.
    server = args.server
    message = "LOOKUP "+str(server)
    client_socket.sendto(message.encode(), disc_port)
    message, addr = client_socket.recvfrom(1024)
    words = message.decode().split()
    if(words[0] == "NOTOK"):
