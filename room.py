...
def signal_handler(sig, frame):
    ...
    room_socket.sendto(message.encode(), disc_port)
...
def process_message(message, addr, room_socket):
    ...
    if (words[0] == "NOTOK"):
        print("Problem with discovery service")
        signal.raise_signal()

    #OK discovery service
...
def main():
    ...
    items = args.item
    message = "REGISTER room://localhost:"+str(port)+" "+name
    room_socket.sendto(message.encode(), disc_port)
    message, addr = room_socket.recvfrom(1024)
    process_message(message.decode(), addr, room_socket)
